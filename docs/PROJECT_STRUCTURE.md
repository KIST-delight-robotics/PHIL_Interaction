...
- `console_log.py` — output split layer: redirects stdout/stderr to
  `runtime_log/brainlog_*.txt` and keeps only `console()`/`console_input()`
  conversation lines on the terminal (`runtime_log/` is a git-ignored artifact dir)
